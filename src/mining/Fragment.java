			...
			if (node instanceof EGroumActionNode){
				if (node.isCoreAction()) {
					if (labels.contains(node.getLabel()))
						continue;
					HashSet<EGroumNode> ins = new HashSet<>();
					boolean found = false;
					for (EGroumEdge e : node.getInEdges()) {
						if (e.getSource() instanceof EGroumDataNode && e instanceof EGroumDataEdge && ((EGroumDataEdge) e).getType() == Type.PARAMETER) {
							if (nodes.contains(e.getSource())) {
								found = true;
								break;
							}
